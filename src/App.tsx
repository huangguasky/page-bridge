...
function Kindle({config,edit}:{config:SmtpConfig,edit:()=>void}) {
 const [files,setFiles]=useState<string[]>([]),[selected,setSelected]=useState(config.recipients),[state,setState]=useState('')
 const pick=async()=>setFiles(await window.pageBridge.pickFiles(true)); const send=async()=>{if(!files.length)return;setState('正在连接邮件服务器并发送…');try{await window.pageBridge.sendKindle(files,selected);setState('发送成功！Kindle 送达通常需要几分钟，请稍后刷新书库。');setFiles([])}catch(e){setState(`发送失败：${e instanceof Error?e.message:String(e)}`)}}
 return <section><Top title="传输到 Kindle" sub="支持 EPUB、PDF、DOCX、TXT 和常见图片格式"/><div className="toolbar"><button className="link" onClick={edit}>⚙ 修改邮件配置</button></div><div className="drop" onClick={pick}><div className="drop-icon">＋</div><h3>{files.length?`已选择 ${files.length} 个文件`:'点击选择书籍文件'}</h3><p>也可一次选择多个文件 · 单封邮件大小受邮箱服务商限制</p></div>{files.length>0&&<div className="panel list">{files.map((f,i)=><div className="file-row" key={f}><span>📄</span><b>{basename(f)}</b><button onClick={()=>setFiles(files.filter((_,x)=>x!==i))}>移除</button></div>)}<h4>发送到</h4>{config.recipients.map(x=><label className="recipient" key={x}><input type="checkbox" checked={selected.includes(x)} onChange={e=>setSelected(e.target.checked?[...selected,x]:selected.filter(y=>y!==x))}/>{x}</label>)}<button className="primary" disabled={!selected.length} onClick={send}>发送到 Kindle</button></div>}{state&&<div className={state.startsWith('发送失败')?'notice bad':'notice'}>{state}</div>}</section>
}

function Phone() {
 const [files,setFiles]=useState<string[]>([]),[share,setShare]=useState<ShareInfo|null>(null),[error,setError]=useState('')
 useEffect(()=>()=>{window.pageBridge.stopShare()},[])
 const pick=async()=>{await window.pageBridge.stopShare();setShare(null);setFiles(await window.pageBridge.pickFiles(false))}; const start=async()=>{try{setShare(await window.pageBridge.startShare(files))}catch(e){setError(e instanceof Error?e.message:String(e))}}; const stop=async()=>{await window.pageBridge.stopShare();setShare(null)}
 return <section><Top title="传输到手机" sub="电脑和手机需要连接同一个 Wi-Fi"/>{!share?<><div className="drop phone-drop" onClick={pick}><div className="drop-icon">＋</div><h3>{files.length?`已选择 ${files.length} 个文件`:'选择要发送的文件'}</h3><p>不限格式，文件不会上传到互联网</p></div>{files.length>0&&<div className="panel list">{files.map(f=><div className="file-row" key={f}><span>📎</span><b>{basename(f)}</b></div>)}<button className="primary" onClick={start}>建立临时共享通道</button></div>}</>:<div className="share panel"><div><div className="live">● 正在共享</div><h2>用手机扫描二维码</h2><p>或在手机浏览器输入以下地址：</p><code>{share.url}</code><p className="muted">手机点击下载后，文件会通过当前 Wi-Fi 直接传输。请保持此窗口打开。</p><button className="danger" onClick={stop}>结束共享</button></div><img src={share.qrDataUrl} alt="手机接收地址二维码"/></div>}{error&&<p className="error">{error}</p>}</section>
}
